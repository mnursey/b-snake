...
BAD_POSITIONS = [WALL, SNAKE, DANGER, SNAKE_HEAD]
...
def run_ai(data):
    ...
    waypoints = generate_waypoints(grid, BAD_POSITIONS, interest_points)
    end = time.time()
    ...
    if not found_tail:
        ...
        if path is not None:
            if len(path) > 1:
                ...
                print(path)
                move = smart_direction(my_snake_head, path[1], grid, BAD_POSITIONS, my_snake_overlapping)
