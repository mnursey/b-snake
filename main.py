from graph_algorithms import find_path, generate_waypoints, link_waypoints, enough_space, flood_fill
import bottle
import os
import time
...
def path_to_tail(my_snake_head, my_snake_tail, waypoints, links, grid):
    ...
    '''if enemy_near_tail(my_snake_head, my_snake_tail, grid):
        # and distance(my_snake_head, my_snake_tail) <= 2
        return None'''
    ...
    for snake in snakes:
        ...
        for n in head_neighbours:
            ...
            if path is not None:
                ...
                if len(neighbours(enemy_head, grid, DEATH_POSITIONS)) == 0:
                    taunt = 'Say goodbye!'
                    print('Moving in the kill enemy')
                    grid[block_pos[0]][block_pos[1]] = temp_hold
                    return path
